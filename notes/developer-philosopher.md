...
## Placement (decided): book-end

This beat is split across the essay so the credential is established before it's spent and the *mechanism* lands where it does the most damage.

**Front (the opening credential — short).** Establish the stance and the standpoint, name "the permission system" once, and *withhold* the walk-through. End on a forward hook ("I'll show you the keys later"). This sets the essay's first-person voice immediately and earns the authority the argument will draw on. Draft:

> I build the things I'm about to criticize — not as a hobby beside the thinking, but as the thinking itself. The ariel app is not a project I wrote an essay about; it is an argument stated in software, and this is the same argument in prose. I work as an integrated developer-philosopher, which is a clumsy phrase for a simple discipline: I only make claims about platforms whose mechanism I could build. That constraint is also a credential. So when I say these systems have an owner who is hidden on purpose, I am not inferring it from how the feed *feels* — I am reporting it from inside, from the seat where you grant yourself the keys. I'll show you the keys later. For now, hold onto one fact: the person making this argument is someone who has held them.

**Back (the pivot — full machinery).** At the Foucault beat (section 5), cash the hook: deliver *permissions are sovereignty* and *the stack of thrones* in full. This is where the essay turns from "power feels ownerless" (the seven thinkers' theory of the effect) to "there is, in fact, an owner, and here is his permission table" (the builder's mechanism). The opening promised the keys; the pivot hands them over. The objections (esp. "you're a platform owner condemning platform owners") also belong here, folded into the Han discomfort beat.

More than two placements, this is the **voice**: the essay's first person speaks *from* this position throughout. It is also the project's one-line positioning — not "a portfolio and some writing," but a single practice: *arguments that compile.*
