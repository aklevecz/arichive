<script>
	...
	const src = images[0] ? `${imagePrefix}/${images[0]}` : 'projects/bao/bao.jpeg';
	const imgSrc = `${host}/${src}`;
</script>

<svelte:head>
	<title>{project?.name + ' | Ariel Klevecz'}</title>
	<meta name="description" content={project?.description} />
	<meta property="og:title" content={project?.name + ' | Ariel Klevecz'} />
	<meta property="og:description" content={project?.description} />
	<meta property="og:image" content={imgSrc} />
	<meta property="og:url" content={`${host}/${project?.id}`} />
	<meta name="twitter:title" content={project?.name + ' | Ariel Klevecz'} />
